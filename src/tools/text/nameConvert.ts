...
// 从 ctool 复制 (helper/nameConvert.ts) — 零修改

const spaceCaseToPascalCaseSpace = 'spaceCaseToPascalCaseSpace'
const pascalCaseSpaceToKebabCase = 'pascalCaseSpaceToKebabCase'
const kebabCaseToUpperSnakeCase = 'kebabCaseToUpperSnakeCase'
const upperSnakeCaseToPascalCase = 'upperSnakeCaseToPascalCase'
const pascalCaseToCamelCase = 'pascalCaseToCamelCase'
const camelCaseToLowerSnakeCase = 'camelCaseToLowerSnakeCase'
const lowerSnakeCaseToSpaceCase = 'lowerSnakeCaseToSpaceCase'
...
class Convent {
    ...
    private convert() {
        ...
        for (let i = 0; i < executeMethods.length; i++) {
            this.str = str.trim()
            str = (this as any)[executeMethods[i]](str);
            this.result[executeMethods[i]] = str;
        }
    }

    spaceCase() { return this.result[lowerSnakeCaseToSpaceCase] || "" }
    pascalCaseSpace() { return this.result[spaceCaseToPascalCaseSpace] || "" }
    kebabCase() { return this.result[pascalCaseSpaceToKebabCase] || "" }
    upperSnakeCase() { return this.result[kebabCaseToUpperSnakeCase] || "" }
    pascalCase() { return this.result[upperSnakeCaseToPascalCase] || "" }
    camelCase() { return this.result[pascalCaseToCamelCase] || "" }
    lowerSnakeCase() { return this.result[camelCaseToLowerSnakeCase] || "" }
}
